...
void Game :: handleInput(const Interface & ui)
{
   if (lander.isAlive() && !lander.isLanded())
   {
	   if (!ui.isDown() && !ui.isLeft() && !ui.isRight() && lander.canThrust())
	   {
   ...
   if (lander.isLanded() && !soundHasPlayed)
   {
	   PlaySound("Celebration.wav", NULL, SND_SYNC);
	   soundHasPlayed = true;
   }
   if (!lander.isAlive() && !soundHasPlayed)
   {
	   PlaySound("Racist.wav", NULL, SND_SYNC);
